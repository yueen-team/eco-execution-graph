...
| `build_graph.py` | 合成全量图,双形态输出 | 校验:每边 source_ref 可解析、node/edge/source tier 必填,法律判断边 legal_basis_status 必填 |
| `export.py` | tier 过滤导出 + manifest/hash | 前置调用 validate_no_private_leak,失败即中止 |
| `gap_report.py` | 三类缺口检测 | 管理经验类问题不得静默挂接低置信法条;pitfall_instance 不得进入共有报告 |
...
- quality 默认规则维护在 `pipeline/quality_rules.json`(P1 建立)。
