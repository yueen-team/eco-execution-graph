param(
  [string]$OutputDir = "graph-ui/dist-cloudbase-static-internal",
  [string]$PublicBase = "/eco-execution-graph-internal/",
  [string]$GraphApiBase = "https://www.yueen.cc/container-eco-execution-graph"
)
...
$oldApiBase = $env:VITE_GRAPH_API_BASE
$env:ECO_GRAPH_UI_BASE = $PublicBase
$env:VITE_GRAPH_API_BASE = $GraphApiBase
try {
  pnpm --dir graph-ui build | Out-Host
} finally {
  $env:ECO_GRAPH_UI_BASE = $oldBase
  $env:VITE_GRAPH_API_BASE = $oldApiBase
}
...
# Internal review shell is still a static package: no private graph or review JSON is shipped.
...
$hashes = @()
foreach ($name in @("index.html", "app.html", "demo-data/full-graph.json", "demo-data/full-cards.json", "demo-data/deploy-policy.json")) {
...
$manifestPath = Join-Path $reports "cloudbase-static-internal-package.json"
...
$lines = @(
  ...
  "- review_api_base: ``$($manifest.review_api_base)``",
...
$lines -join "`n" | Set-Content (Join-Path $reports "cloudbase-static-internal-package.md") -Encoding UTF8

if ($findings.Count -gt 0) {
  throw "CloudBase internal package contains blocked markers. See reports/cloudbase-static-internal-package.json"
}
