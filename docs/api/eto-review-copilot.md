...
| `graph-api/src/copilot-llm.js` | LLM critic:调 DeepSeek + RAG,产结构化异议 1/3/6/7;fail-closed | 是(仅凭证 lane) |
...
- **输入**:system(四条铁律 + 降级规则 + 输出 JSON schema)+ 脱敏 review item + graph context(`buildGraphContextResponse` 结果)+ 候选法条 RAG 原文(`rag_resolve`)。
- **输出**:强制 JSON,只允许 §7 的 `异议[]` 结构;`response_format` 走 JSON,服务端再用 schema 校验;**任何缺 trace 或 trace 不在本次 graph context 内的 finding 一律丢弃**(防幻觉法条)。
- **fail-closed 矩阵**:

| 情况 | 副驾行为 |
|---|---|
| 无 TokenHub/RAG 密钥(本地/CI) | 只返回确定性 backbone,`门禁` 不降级(本就无 LLM 段) |
| RAG 取文失败 | 涉及法条语义的异议(1/3)降级为"需人工复核法条",`门禁=partial` |
