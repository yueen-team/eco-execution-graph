...
- **输入(红线分域装配,`buildCopilotPrompt`)**:
  ...
  - **「法条引用」段(citation 域)**:`projectCitations(citations, graphContext)` 产出 —— **允许法条原文**(`tc3-rag-client.js` 经 LKE `RetrieveKnowledge` 取的 `excerpt`),但只取本轮**已审核来源**——精确 `rag_doc_ref` **或**版本不敏感的**法规标识**(标准号如 `GB 18597`、法名+条款号如 `固体废物污染环境防治法 第七十七条`)命中本轮 `law_refs/tech_spec_refs` 且不在 `blocked_refs`(`approvedRagRefs`/`citationHitsIdentity`;candy 2026-06-30 定:真实知识库记录常缺 opaque `rag_doc_ref` 或其值与图谱来源节点不同源,故改用稳定的法规标识做键;条款号匹配须法名 + 条款号俱全,防过宽)、≤2000 字符截断、逐条过 `scanCitationForbidden` + private-tier;脏的【该条原文置 `null`】(降级该条,不整体抛)。`assertCitationSegmentClean` 兜底:禁私有 / 企业 / 坐标 / 密钥 / 照片,命中即抛。
