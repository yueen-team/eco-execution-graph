...
// ── 法规标识匹配(candy 2026-06-30):真实 KB 记录 rag_doc_ref 缺失/不同源时,按标准号/条款号对接已审核来源 ──
const STD_LAW_TEXT = "第四条 危险废物的容器和包装物以及贮存场所应当按照规定设置危险废物识别标志。";

/** 已审核 tech_spec 来源带 standard_no（GB 18597），其 rag_doc_ref 与真实 KB 文档 id 不同源。 */
function ctxWithTechSpec() {
  const ctx = cleanCtx();
  ctx.tech_spec_refs = [{
    node_id: "spec:gb18597", node_type: "tech_spec",
    title: "GB 18597 危险废物贮存污染控制标准·标签管理", standard_no: "GB 18597",
    rag_doc_ref: "tencent-lke://spec/gb18597",
  }];
  return ctx;
}

test("法规标识:rag_doc_ref=null 但标准号命中已审核 tech_spec(GB 18597,版本不敏感)→ 原文进 prompt、rag_available=true", async () => {
  const fetchSpy = fetchReturning({ "异议": [] });
  // 真实 KB 风格:rag_doc_ref=null,标题/定位带标准号 + 版本后缀。
  const ragFetch = async () => [{ rag_doc_ref: null, title: "危险废物贮存污染控制标准(GB 18597—2023代替GB 18597-2001)", locator: "GB 18597--2023", excerpt: STD_LAW_TEXT }];
  const result = await llmCritique({ item: cleanItem(), graphContext: ctxWithTechSpec(), env: KEYED_ENV, fetchImpl: fetchSpy, ragFetch });
  assert.equal(result.rag_available, true, "标准号匹配应放行已审核来源法条原文(GB 18597 命中 GB 18597—2023)");
  assert.equal(JSON.stringify(fetchSpy.calls[0].init.body).includes(STD_LAW_TEXT), true, "已审核来源原文应真进 prompt");
});

test("法规标识:标准号未被图谱审核(HJ 9999)→ 丢弃、原文不进 prompt、rag_available=false", async () => {
  const fetchSpy = fetchReturning({ "异议": [] });
  const ragFetch = async () => [{ rag_doc_ref: null, title: "某未审核标准 HJ 9999—2020", locator: "HJ 9999", excerpt: STD_LAW_TEXT }];
  const result = await llmCritique({ item: cleanItem(), graphContext: ctxWithTechSpec(), env: KEYED_ENV, fetchImpl: fetchSpy, ragFetch });
  assert.equal(result.rag_available, false, "未被图谱审核的标准号不得放行");
  assert.equal(JSON.stringify(fetchSpy.calls[0].init.body).includes(STD_LAW_TEXT), false, "未审核来源原文绝不进 prompt");
});

test("法规标识:已审核来源被 blocked_refs(按标准号)拦截 → 丢弃、rag_available=false", async () => {
  const fetchSpy = fetchReturning({ "异议": [] });
  const ctx = ctxWithTechSpec();
  ctx.blocked_refs = [{ node_id: "spec:gb18597", standard_no: "GB 18597", title: "GB 18597" }];
  const ragFetch = async () => [{ rag_doc_ref: null, title: "GB 18597—2023", locator: "GB 18597--2023", excerpt: STD_LAW_TEXT }];
  const result = await llmCritique({ item: cleanItem(), graphContext: ctx, env: KEYED_ENV, fetchImpl: fetchSpy, ragFetch });
  assert.equal(result.rag_available, false, "blocked 标准号即便命中已审核来源也必须拦截");
});

test("法规标识:法名+条款号命中已审核 law_ref(rag_doc_ref=null)→ 放行;同号但法名不同(防过宽)→ 不放行", async () => {
  const ctx = cleanCtx();
  ctx.law_refs = [{ node_id: "law:swl:art77", title: "固体废物污染环境防治法 第七十七条", law_name: "固体废物污染环境防治法", article_no: "第七十七条", rag_doc_ref: "tencent-lke://law/swl/art77" }];

  const okFetch = fetchReturning({ "异议": [] });
  const hit = async () => [{ rag_doc_ref: null, title: "固体废物污染环境防治法 第七十七条", locator: "第七十七条", excerpt: STD_LAW_TEXT }];
  const r1 = await llmCritique({ item: cleanItem(), graphContext: ctx, env: KEYED_ENV, fetchImpl: okFetch, ragFetch: hit });
  assert.equal(r1.rag_available, true, "法名+条款号命中已审核 law_ref 应放行");

  const noFetch = fetchReturning({ "异议": [] });
  // 大气法的第七十七条:条款号相同、法名不同 → 不得误放行。
  const miss = async () => [{ rag_doc_ref: null, title: "大气污染防治法 第七十七条", locator: "第七十七条", excerpt: STD_LAW_TEXT }];
  const r2 = await llmCritique({ item: cleanItem(), graphContext: ctx, env: KEYED_ENV, fetchImpl: noFetch, ragFetch: miss });
  assert.equal(r2.rag_available, false, "法名不同的同号条款不得误放行(防过宽匹配)");
});
