...
## 1. 总体架构:RAG 原文层、知识治理层、一张图、四个消费面

```
                  ┌───────────────────────────────────────────────┐
   原文与上游       │                本仓库(图谱与治理层)             │      消费面
                  │                                               │
 腾讯云 RAG 知识库 ─┼→ knowledge_governance registry/candidates ───┼──→ DeepSeek 专家智能体
 (法规/规范全文)    │   (metadata-only,不保存 RAG Content)          │
                  │                    │                          │
                  │                    ▼                          │
 EcoCheck 蒸馏流 ─→ pipeline/ingest  ─ data/candidates             │ ① 演示驾驶舱(graph-ui ego 视图)
 (semantic_event   │  (CANDIDATE)      │                          │ ② 共有导出包(软著/培训/执法工具)
  _outbox v2)      │                    ▼                          │ ③ 缺口报告(双向,给主任团队)
                  │             data/approved                     │ ④ 上下文装配 API(第二刀,回灌
 eco-semantic-kb ─→ pipeline/import     │                          │    EcoCheck 月报,docs/api)
 approved          │  (骨架挂载点)       ▼                          │
 baselines         │             eco-execution-graph               │
                  │  (关系语义、现场经验、法条瘦节点)              │
 semantic-         │                    │                          │
 profile-lab       │                    ▼                          │
 contracts ───────→ schema/(node/edge/source/card/governance)      │
                  │                    │                          │
...
核心原则(继承 2026-06-01 审计结论):**一张图、溯源优先、现场为源、成效定权、薄切发车。** 知识治理中层只治理文档身份、版本、去重、废止状态、候选反馈和发布包,不保存法规/规范全文,不直接写腾讯云知识库。
...
知识治理中层新增三类 metadata-only 对象,用于把 RAG 文档、图谱反馈和消费侧发布分开治理:

| 对象 | 职责 | 默认路径 |
|---|---|---|
| `KnowledgeDocument` | 法规/技术规范/政策/指南的文档级注册表,记录 `rag_doc_ref`、hash、版本、废止/替代、去重组与审核状态 | `data/knowledge-governance/doc-registry/knowledge-documents.json` |
| `KnowledgeGovernanceCandidate` | 待审核治理候选,包括去重、定位补全、废止状态、图谱专家经验候选和法律映射复核 | `data/knowledge-governance/candidates/governance-candidates.json` |
| `PublicationBundle` | 面向 `expert_agent`(小悦)、`ecocheck`、`ecodoc`(EcoCheck 环保体检报告 worker)、`internal_review` 的只读发布包 | `data/knowledge-governance/publications/*.json` |

这些对象不得包含 RAG `Content`、法规全文、技术规范全文、企业私有证据、GPS、照片路径或原始报告片段。图谱成果可以回流为候选,但不能自动改写法规/规范原文知识库。ETO 审核进图是小悦/EcoCheck/EcoCheck 环保体检报告 worker 消费的主审核;出口只跑机器 policy gate,不追加一道人审。

## 3. 模块边界

| 模块 | 职责 | 禁区 |
|---|---|---|
| `schema/` | JSON Schema 单一事实源;`pnpm graph:schema:blocking` 会用 node/edge/source schema 校验当前上游、全量、shared 图数据集 | 改 schema 必须配 ADR 或 specs 更新 |
| `pipeline/` | ingest(蒸馏流→CANDIDATE)/ import(baseline→骨架)/ normalize(归一)/ build(成图)/ export(tier 过滤导出)/ gap_report(缺边检测);P3 起 `p2p3_common.py` 只做兼容 facade,路径、IO、record factory、ETO override、上游 lock helper、schema validation 拆入独立模块 | 不得绕过人工审核把 CANDIDATE 直接置 APPROVED;不得在导出中携带 private 数据 |
...
### 4.2.1 知识治理中层(P0.6,本地离线)

```
RAG citation metadata + 图谱 law/standard thin refs
        → pipeline/knowledge_governance.py build-registry
        → KnowledgeDocument registry
        → generate-candidates(定位补全/去重/废止/图谱反馈)
        → publish-bundles(小悦/EcoCheck/EcoCheck 环保体检报告 EcoDoc worker/internal_review)
```

治理层只处理 metadata 与 trace。source-level citation 会进入 `locator_patch` 候选;图谱现场经验进入 `graph_expert_candidate` 或 `legal_mapping_review` 候选;废止或被替代文档进入 publication `blocked_items`。v1 不调用腾讯云文档管理 API,也不写 EcoCheck 生产库。

### 4.2.2 专家智能体/小程序双接口消费

专家智能体和 EcoCheck 小程序不必把所有知识回填进腾讯云 RAG。它们同时调用两个只读接口:

1. **图谱 API**(`/api/graph/context`):按问题类型、场景或关键词返回 ETO 已审核图谱上下文、`law_refs`、`tech_spec_refs`、blocked refs 和 trace。
2. **腾讯云 RAG API**:按图谱返回的 `rag_doc_ref`、条款号或标准号取法规/规范原文。

图谱负责匹配和解释瘦条款,RAG 负责取回原文。若图谱 API 返回 `blocked_refs` 或 `legal_basis_status` 不是 `official_confirmed/internal_reviewed`,生成内容必须降级为内部提示或管理建议。
...
| **P0.6 知识治理中层** | RAG citation metadata + 图谱 thin refs → 文档注册表、治理候选、消费侧发布包 | 去重/废止/定位补全有候选队列,小悦/EcoCheck/EcoCheck 环保体检报告 worker 只读 approved/human_reviewed bundle |
...
| **P5 第二刀:回灌** | 上下文装配 API → EcoCheck 月报/环保体检报告 EcoDoc worker 接地;成效回写 confidence 飞轮 | 报告引用可追溯,飞轮跑通 |
